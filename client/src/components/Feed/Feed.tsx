                ...
                <div className="flex flex-col gap-3 w-full">
                    ...
                    <div className="flex flex-wrap gap-2 mt-1">
                        {listing.estimatedValue ? (
                            <span className="text-emerald-50 text-xs font-bold bg-emerald-500/80 px-3 py-1.5 rounded-[12px] backdrop-blur-md border border-emerald-400/50 shadow-sm">
                                Worth: KES {listing.estimatedValue.toLocaleString()}
                            </span>
                        ) : null}
                        <span className="text-white/90 text-xs font-bold bg-white/10 px-3 py-1.5 rounded-[12px] backdrop-blur-md border border-white/20 shadow-sm">
                            Wants: {listing.wantItems || "Offers"}
                        </span>
                        <span className="text-white/90 text-xs font-bold bg-white/10 px-3 py-1.5 rounded-[12px] backdrop-blur-md border border-white/20 shadow-sm flex items-center gap-1">
                            <span className="text-[10px]">📍</span> {listing.location || 'Nearby'}
                        </span>
                    </div>
                    ...
                    <div className="flex items-center gap-3 mt-4">
                        <button 
                            onClick={onPropose}
                            className="flex-1 py-4 bg-white text-black font-extrabold text-[15px] rounded-[20px] shadow-[0_8px_30px_rgba(255,255,255,0.2)] transform transition hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2"
                        >
                            Offer Swap
                        </button>
                        
                        <button onClick={handleSave} className="w-14 h-14 bg-white/10 backdrop-blur-md rounded-[20px] border border-white/20 flex items-center justify-center transition-colors hover:bg-white/20 active:scale-95 shrink-0">
